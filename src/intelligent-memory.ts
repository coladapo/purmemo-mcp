#!/usr/bin/env node
import type { IntelligentContext, ProgressIndicators, Relationships, ClusteringHints, Methodology } from './types.js';
 ...
 */
function extractProjectContext(content: string): IntelligentContext {
  const context: IntelligentContext = {
    ...
    related_work: [] as string[],
    clustering_hints: {
      belongs_to_project: null,
      belongs_to_component: null,
      belongs_to_feature: null,
      phase_number: null,
      primary_intent: '',
      work_category: ''
    }
  ...
  for (const [status, pattern] of Object.entries(emojiStatusMap)) {
    if (pattern.test(content)) {
      context.status = status as IntelligentContext['status'];
      break;
    }
  }
  ...
  context.clustering_hints = {
    ...
    phase_number: context.phase ? extractPhaseNumber(context.phase) : null,
 ...
 */
function generateIntelligentTitle(context: IntelligentContext, conversationContent: string): string {
  const {
    project_name,
    project_component,
    feature_name,
 ...
 * }
 */
function extractProgressIndicators(content: string): ProgressIndicators {
 ...
 */
function extractRelationships(content: string): Relationships {
...
function extractPhaseNumber(phase: string): number | null {
...
function detectPrimaryIntent(content: string): string {
...
function detectWorkCategory(component: string | null, content: string): string {
