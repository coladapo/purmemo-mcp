// @ts-nocheck — CLI utility, full typing in follow-up
/**
...
class UniversalAuthManager {
    ...
    async authenticate(options = {}) {
        const { client = 'universal', purpose = 'general', interactive = true } = options;
    ...
    async saveClientSpecific(token, client) {
        switch (client) {
            ...
            case 'mcp':
                ...
                fs.writeFileSync(path.join(this.configDir, 'auth.json'), JSON.stringify(mcpAuth, null, 2));
                break;
            case 'npm':
                // Add to bashrc/zshrc
            ...
            case 'cursor':
                ...
                fs.writeFileSync(path.join(vscodeDir, 'purmemo.json'), JSON.stringify({ api_key: token }, null, 2));
                break;
        }
    }
    /**
