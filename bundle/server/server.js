    ...
    if (REMOTE_MODE) {
        ...
        setInterval(() => {
            ...
            for (const key of Object.keys(oauthStateStorage)) {
                if (now - oauthStateStorage[key].createdAt > 600_000)
                    delete oauthStateStorage[key];
            }
            for (const key of Object.keys(refreshTokenStore)) {
                if (now - refreshTokenStore[key].createdAt > 86_400_000)
        ...
        app.post('/login', async (req, res) => {
            ...
            try {
                ...
                if (authData.refresh_token)
                    refreshTokenStore[apiKey] = { token: authData.refresh_token, createdAt: Date.now() };
